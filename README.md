...
Reports **orphan modules** — `*.msd` / `*.mpl` / `*.devkit` files present on disk but not registered in any
...
(`*.msd` / `*.mpl` / `*.devkit`), and the per-project `.mps/modules.xml` / `.mps/libraries.xml`. A zero-byte file here
is almost always the result of a botched save, merge, or checkout; MPS won't load it, silently dropping whatever it
held.

### `mps-check-module-naming`

Checks that every module descriptor (`*.msd` / `*.mpl` / `*.devkit`) agrees with its layout on disk. The directory and
the file must be be named after the full module name: `com.example.foo` must be located in
`com.example.foo/com.example.foo.mpl` (likewise for `.msd` or `.devkit`).
